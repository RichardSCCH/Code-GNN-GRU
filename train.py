...
def limitgpu(maxmem):
    ...
    if gpus:
        ...
        try:
            for g in gpus:
                tf.config.experimental.set_virtual_device_configuration(g, [
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size', dest='batch_size', type=int, default=80)
    ...
    print('validation data size {}'.format(valsteps * batch_size))
    print('------------------------------------------')
    ...
    model.fit(gen, steps_per_epoch=steps, epochs=epochs, verbose=1, max_queue_size=4,
